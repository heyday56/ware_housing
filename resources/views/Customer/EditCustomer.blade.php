  ...
  <div class="container flex min-h-screen w-fit">
    <div
      ...
      <nav>
        ...
        <div class="mt-4">
          <h6 class="text-blue-600 text-sm font-bold px-4">Edit Customer</h6>
          <ul class="mt-2">
            <li>
    ...
    <div class="table-container flex-1 p-5 ml-[20px] w-screen">
      <div class="m-3"></div>
      <form class="bg-white rounded-lg p-10" method="post" action="{{ url('/customer/update/' . $customer->id) }}">
        @csrf
        <div class="grid gap-6 mb-6 md:grid-cols-2">
          ...
          <div>
            ...
            <input type="text" id="phone_no" name="phone_no" value="{{ $customer->phone_no }}"
              class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
          ...
          <div>
            <label for="date_joined" class="block mb-2 text-sm font-medium text-gray-900">Joined
              Date</label>
            <input type="date" id="date_joined" name="date_joined" value="{{ $customer->date_joined }}"
              class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
              placeholder="Date Joined" required />
          </div>
          <div>
            <label for="address" class="block mb-2 text-sm font-medium text-gray-900">Address</label>
            <input type="address" id="address" name="address" value="{{ $customer->address}}"
              class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
              placeholder="Date Joined" required />
          </div>
          <button type="submit"
